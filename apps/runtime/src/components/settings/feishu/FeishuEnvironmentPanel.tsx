...
import { normalizeLegacyChannelDisplayCopy } from "../channels/channelRegistryHelpers";
...
export function FeishuEnvironmentPanel({
  ...
  const environmentError = normalizeLegacyChannelDisplayCopy(feishuEnvironmentStatus?.error);
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-4 space-y-3">
      <div>
        ...
        <div className="text-xs text-gray-500 mt-1">飞书平台适配器使用宿主机自己的 Node.js；当前要求 Node.js {requiredNodeMajor}+。</div>
      </div>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <div className="rounded border border-gray-100 bg-gray-50 px-3 py-3">
          ...
          <div className="mt-1 text-[11px] text-gray-500">{environmentError || "完成环境检查后即可继续后续步骤"}</div>
        </div>
      </div>

...
describe("ChatView session resilience", () => {
  ...
  test("preserves slash command text when sending the initial message", async () => {
    vi.useFakeTimers();

    function InitialMessageHarness() {
      const [initialMessage, setInitialMessage] = useState("/pm_summary --employee xt --date 2026-03-27");
    ...
    expect(invokeMock).toHaveBeenCalledWith("send_message", {
      request: {
        sessionId: "sess-initial-slash",
        parts: [{ type: "text", text: "/pm_summary --employee xt --date 2026-03-27" }],
      },
    });
  });
  ...
  test("preserves slash command text when sending from the chat composer", async () => {
    ...
    render(
      <ChatView
        ...
        sessionId="sess-slash-composer"
      />
    );

    const composer = await screen.findByPlaceholderText("输入消息，Shift+Enter 换行...");
    fireEvent.change(composer, { target: { value: "/pm_summary --employee xt --date 2026-03-27" } });
    fireEvent.click(screen.getByRole("button", { name: "发送" }));
  ...
  test("debounces session draft persistence while the user is typing", async () => {
    render(
      <ChatView
        skill={{
          id: "builtin-general",
    ...
    const composer = screen.getByPlaceholderText("输入消息，Shift+Enter 换行...");
    fireEvent.change(composer, { target: { value: "第一次输入" } });

    expect(setItemSpy).not.toHaveBeenCalledWith(
      "workclaw:session-draft:sess-debounce-draft",
      "第一次输入",
    );

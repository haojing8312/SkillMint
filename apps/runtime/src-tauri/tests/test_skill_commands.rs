...
use runtime_lib::commands::skills::{
    create_local_skill_in_dir, import_local_skills_to_pool, render_local_skill_preview_in_dir,
};
use std::path::Path;
...
async fn create_local_skill_rejects_existing_directory() {
    ...
    let first = create_local_skill_in_dir(
        "File Organizer",
        "organize files",
        "需要整理大量文件时",
        base,
    ...
    let err = create_local_skill_in_dir(
        "File Organizer",
        "organize files",
        "需要整理大量文件时",
        base,
...
#[tokio::test]
async fn render_local_skill_preview_in_dir_can_represent_default_input_values() {
    let tmp = tempfile::tempdir().expect("create temp dir");
    let preview = render_local_skill_preview_in_dir(
        "expert-skill",
        "",

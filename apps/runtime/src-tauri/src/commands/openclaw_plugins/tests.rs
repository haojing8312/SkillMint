...
use super::installer_session::{
    ...
    ensure_openclaw_cli_shim, infer_installer_prompt_hint, prepend_env_path,
};
use super::plugin_host_service::{
    apply_command_search_path, build_effective_path_entries,
    build_plugin_host_fixture_root_from_runtime_root, collect_windows_node_command_candidates,
    derive_channel_capabilities, derive_feishu_plugin_environment_status,
    parse_windows_registry_path_output, resolve_windows_node_command_path,
};
...
use crate::runtime_bootstrap::{default_runtime_root_bootstrap, RuntimeBootstrapLocation};
use crate::runtime_environment::{ManagedRuntimeEnvironment, RuntimeEnvironment};
...
fn build_test_app_with_runtime_root(runtime_root: &Path) -> tauri::App<tauri::Wry> {
    let runtime_environment = RuntimeEnvironment {
        bootstrap_location: RuntimeBootstrapLocation {
            bootstrap_dir: runtime_root.join("bootstrap"),
            bootstrap_path: runtime_root.join("bootstrap").join("runtime-root.json"),
        },
        bootstrap: default_runtime_root_bootstrap(runtime_root),
        paths: RuntimePaths::new(runtime_root.to_path_buf()),
    };

    tauri::Builder::default()
        .manage(ManagedRuntimeEnvironment(Arc::new(runtime_environment)))
...
fn write_fake_npm_command(bin_dir: &Path, marker: &Path) {
    let script_path = bin_dir.join("npm.cmd");
    let script = format!(
        "@echo off\r\n\
             echo called>\"{marker}\"\r\n\
             mkdir \"%CD%\\node_modules\\.bin\" 2>nul\r\n\
             echo @echo off>\"%CD%\\node_modules\\.bin\\feishu-plugin-onboard.cmd\"\r\n\
             exit /b 0\r\n",
        marker = marker.display()
    );
    std::fs::write(script_path, script).expect("write fake npm.cmd");
}

#[cfg(not(target_os = "windows"))]
fn write_fake_npm_command(bin_dir: &Path, marker: &Path) {
    let script_path = bin_dir.join("npm");
    let script = format!(
            "#!/usr/bin/env sh\n\
             echo called > \"{marker}\"\n\
             mkdir -p \"$PWD/node_modules/.bin\"\n\
             printf '#!/usr/bin/env sh\nexit 0\n' > \"$PWD/node_modules/.bin/feishu-plugin-onboard\"\n\
             chmod +x \"$PWD/node_modules/.bin/feishu-plugin-onboard\"\n\
             exit 0\n",
            marker = marker.display()
        );
    std::fs::write(&script_path, script).expect("write fake npm");
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(&script_path, permissions).expect("chmod fake npm");
    }
}

fn wait_for_installer_session_exit(state: &OpenClawLarkInstallerSessionState) {
    for _ in 0..40 {
        if !current_openclaw_lark_installer_session_status(state).running {
            return;
        }
        std::thread::sleep(Duration::from_millis(100));
    }
    panic!("installer session did not exit in time");
}
...
async fn feishu_advanced_settings_treats_blank_rows_as_unset_defaults() {
    ...
    for key in [
        ...
        set_app_setting(&pool, key, "")
            .await
            .expect("set blank app setting");
    }
...
async fn build_feishu_openclaw_config_projects_official_defaults() {
    ...
    .await
    .expect("set dms");
    set_app_setting(&pool, "feishu_footer", "{\"status\":true,\"elapsed\":true}")
        .await
        .expect("set footer");
    set_app_setting(
            &pool,
...
fn installer_prompt_hint_explains_poll_waiting_states() {
    ...
    assert_eq!(
        infer_installer_prompt_hint("[DEBUG] Poll result: {\"error\":\"authorization_pending\"}")
            .as_deref(),
...
#[test]
fn derives_environment_status_when_node_version_is_too_old() {
    let status = derive_feishu_plugin_environment_status(
        Ok(Some("v20.11.1".to_string())),
        Ok(Some("10.8.0".to_string())),
        true,
    );
    ...
    assert!(status.node_available);
    assert!(status.npm_available);
    assert_eq!(status.node_version.as_deref(), Some("v20.11.1"));
    assert!(!status.node_version_supported);
    assert_eq!(status.required_node_major, 22);
    assert!(!status.can_install_plugin);
    assert!(!status.can_start_runtime);
    assert_eq!(
        status.error.as_deref(),
        Some("已检测到 Node.js v20.11.1，但飞书官方插件当前要求 Node.js >= v22")
    );
}
...
fn derives_environment_status_when_node_is_missing() {
    let status =
        derive_feishu_plugin_environment_status(Ok(None), Ok(Some("10.8.0".to_string())), true);
...
fn derives_environment_status_when_npm_is_missing() {
    let status =
        derive_feishu_plugin_environment_status(Ok(Some("v22.0.0".to_string())), Ok(None), true);
...
fn ensure_openclaw_cli_shim_rewrites_existing_script() {
    ...
    let script =
        std::fs::read_to_string(shim_dir.join("openclaw-shim.mjs")).expect("read refreshed shim");
...
fn installer_command_prefers_installed_bin_script_when_present() {
    ...
    assert_eq!(
        ...
        Some(&*installer_script.to_string_lossy())
    );
    ...
    assert!(
        !args
            .iter()
            .any(|value| value.contains("@larksuite/openclaw-lark")),
...
#[tokio::test]
async fn installer_session_skips_bootstrapping_tools_when_plugin_bin_exists() {
    ...
    let app = build_test_app_with_runtime_root(&runtime_root);
    let pool = setup_memory_pool().await;

    let fake_bin_dir = temp.path().join("fake-bin");
    let fake_npm_marker = temp.path().join("fake-npm-called.txt");
    std::fs::create_dir_all(&fake_bin_dir).expect("create fake bin dir");
    write_fake_npm_command(&fake_bin_dir, &fake_npm_marker);
    ...
    let plugin_install_path = runtime_root
        ...
        .join("@larksuite")
        .join("openclaw-lark");
    let installer_script = plugin_install_path.join("bin").join("openclaw-lark.js");
    std::fs::create_dir_all(installer_script.parent().expect("installer script parent"))
        .expect("create installer script parent");
    std::fs::write(
        &installer_script,
        "console.log('plugin bin installer'); process.exit(0);\n",
    )
    .expect("write installer script");
    ...
    upsert_openclaw_plugin_install_with_pool(
        ...
        OpenClawPluginInstallInput {
            ...
            install_path: plugin_install_path.to_string_lossy().to_string(),
    ...
    let installer_state = OpenClawLarkInstallerSessionState::default();
    let start_result = start_openclaw_lark_installer_session_with_pool(
        &pool,
        &installer_state,
        OpenClawLarkInstallerMode::Create,
        None,
    ...
    start_result.expect("start installer session");
    wait_for_installer_session_exit(&installer_state);

    let tools_workspace = runtime_root
        .join("openclaw-plugins")
        .join("openclaw-lark")
        .join("workspace")
        .join("installer-tools");
    assert!(
        !fake_npm_marker.exists(),
        "expected plugin bin installer path without bootstrapping npm tools"
    );
    assert!(
        !tools_workspace.exists(),
        "expected installer-tools workspace to stay absent when plugin bin exists"
    );
}
...
async fn sync_feishu_gateway_credentials_from_controlled_state_reads_env_secret() {
    ...
    std::fs::write(state_root.join(".env"), "LARK_APP_SECRET=secret_from_env\n")
...
fn windows_node_candidates_include_nvm_and_common_install_locations() {
    ...
    assert!(candidates
        .iter()
        .any(|path| path.ends_with(Path::new("node.exe"))));
    assert!(candidates
        .iter()
        .any(|path| path == &nvm_link.join("node.exe")));
    assert!(candidates
        .iter()
        .any(|path| path == &nvm_home.join("node.exe")));
}
...
"#,
    ...
    let state =
        OpenClawPluginFeishuRuntimeState(Arc::new(Mutex::new(OpenClawPluginFeishuRuntimeStore {
    ...
    let stdout_thread = std::thread::spawn(move || {
        ...
        for line in reader.lines().map_while(Result::ok) {
            ...
            let _ = handle_openclaw_plugin_feishu_runtime_send_result_event(&state_clone, &value);
        }
    });
...
fn outbound_command_error_fails_pending_request_immediately() {
    ...
    assert!(
        handled,
        "expected command_error event to resolve pending waiter"
    );
    let result = receiver
...
#[tokio::test]
async fn delete_openclaw_plugin_install_removes_plugin_workspace_from_disk() {
    let temp = tempfile::tempdir().expect("tempdir");
    let runtime_root = temp.path().join("runtime-root");
    std::fs::create_dir_all(&runtime_root).expect("create runtime root");
    let app = build_test_app_with_runtime_root(&runtime_root);
    ...
    let plugin_root = runtime_root.join("openclaw-plugins").join("openclaw-lark");
    let plugin_install_path = plugin_root
        .join("workspace")
        .join("node_modules")
        .join("@larksuite")
        .join("openclaw-lark");
    std::fs::create_dir_all(&plugin_install_path).expect("create plugin install path");
    std::fs::write(plugin_install_path.join("package.json"), "{}").expect("write plugin package");
    std::fs::write(
        plugin_root.join("workspace").join("marker.txt"),
        "plugin workspace",
    )
    .expect("write plugin workspace marker");
    ...
    delete_openclaw_plugin_install_with_pool_and_app(&pool, "openclaw-lark", &app.handle())
        .await
        .expect("delete plugin install");

    assert!(
        get_openclaw_plugin_install_by_id_with_pool(&pool, "openclaw-lark")
            .await
            .is_err(),
        "expected plugin install record to be removed"
    );
    assert!(
        !plugin_root.exists(),
        "expected plugin workspace root to be removed from disk"
    );
}

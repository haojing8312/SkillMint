...
WorkClaw 是一个 Hermes-aligned、本地优先的桌面 AI 员工运行时和工作台，目标是让普通用户也能通过对话式交互安装配置、创建技能、加密打包、组织 AI 员工团队，并通过飞书等 IM 在移动端指挥任务。早期实现曾参考 OpenClaw 生态；在当前路线图中，OpenClaw 仅作为历史来源和遗留迁移输入，不再是 WorkClaw 的前向产品架构或兼容目标。
...
- **面向 AI 员工运行时**：围绕 profile runtime、原生 ToolRegistry / toolsets、平台网关和 profile-owned memory / skills / growth / curator 持续演进
...
- **Profile 级长期记忆**：以 profile runtime 为边界组织记忆、技能、成长记录和 curator 证据；旧 `employee_id` / `skill_id` 仅作为迁移期标签和路由别名。
...
- **桌面 + 飞书统一审批总线**：高风险删除、危险命令等动作可在桌面或飞书任一端审批，并支持重启后的待审批恢复。
...
| **WorkClaw (卧龙AI)** | **开源社区** | **Hermes-aligned AI 员工工作台 / 运行时** | **开源免费、数据本地、profile 运行时、原生工具集、技能可加密打包销售** | **完全免费 (Apache 2.0)** |

### WorkClaw 的独特优势

作为 **完全开源、本地优先** 的桌面 AI 员工运行时，WorkClaw 与商业产品相比具有以下核心差异：
...
- ✅ **浏览器自动化**：Playwright 集成用于网页抓取和自动化；当前 Sidecar 路径是迁移期实现，目标是原生 browser provider + ToolRegistry
...
- ✅ **分层记忆管理**：TodoWrite 任务跟踪 + profile-owned memory / skills / growth / curator 方向；旧员工/技能桶仅保留为迁移期别名
...
- **审批总线降级开关**：如需回退到旧桌面确认路径，可在 `app_settings` 中设置 `approval_bus_v1=false`
- **无云依赖**：所有处理均在本地进行
...
- **长期记忆隔离**：下一代运行时以 `profile_id -> profiles/<profile_id>/...` 为 canonical home；旧员工/技能路径仅作为迁移输入或兼容别名
...
- **Legacy transitional sidecar**：Node.js 20+（Playwright、MCP 迁移期实现，目标是 Rust ToolRegistry 原生 provider）
...
│       │   │   └── db.rs     # SQLite schema
│       │   └── tests/        # 集成测试
│       └── sidecar/          # 迁移期 Node.js sidecar（Playwright, MCP），目标是逐步移入原生 runtime
├── packages/
...
- 拓展生态连接：IM 远程调用、原生 toolsets / skillpack 生态与移动端协作。

### Later（长期方向）
- 企业能力：多租户、SSO / RBAC 权限管控、白名单技能安装、受控工作目录、统一审计、企业级 Token 中转与配额限流。
- Agent 进化：EvoMap / GEP / A2A 集成与可追溯演进。
- 开放生态：建设 Hermes-aligned 的 profile、toolset、Skill OS 和平台 adapter 生态。
...
**Claw**：保留对 OpenClaw 生态和“小龙虾团队”意象的历史致敬，但 WorkClaw 的前向架构是 Hermes-aligned 的 AI 员工运行时
...
- OpenClaw 遗留 vendor lane 维护手册（历史/迁移参考）：[docs/maintainers/openclaw-upgrade.md](docs/maintainers/openclaw-upgrade.md)
...
- 感谢 [OpenClaw](https://github.com/openclaw/openclaw) 开源生态提供的历史灵感与早期参考。当前 WorkClaw 路线图已转向 Hermes-aligned、本地优先的 AI 员工运行时；OpenClaw-shaped 代码和文档仅作为遗留迁移输入保留。

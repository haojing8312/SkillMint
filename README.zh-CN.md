...
- 员工保存/删除后，后端会自动对齐飞书长连接：每个「已启用且配置了 App ID/App Secret」的员工都独立监听。
- 应用启动后会自动恢复监听，并进行周期健康检查，异常时自动重试恢复（带退避）。
- 员工页列表显示飞书连接状态点：
  - 绿：该员工长连接运行中且事件 relay 运行中
  - 红：该员工已绑定飞书但连接异常或 relay 异常
  - 灰：员工未启用或未绑定飞书凭据
- 飞书入站消息会携带员工归属并自动映射到桌面会话：不存在则新建，会话存在则复用，然后自动执行任务。

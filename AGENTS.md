...
- The sidecar removal track is documented in `docs/plans/2026-05-11-hermes-aligned-sidecar-removal-roadmap.md`. Treat `apps/runtime/sidecar` as a legacy transitional implementation, not a future product boundary. Do not add new sidecar endpoints or new sidecar-only runtime dependencies; migrate callers toward Rust ToolRegistry, native providers, gateway/platform adapters, and profile-owned runtime state.
- OpenClaw-related code, names, vendor sync lanes, browser compatibility wrappers, and directory shapes are legacy migration inputs only. Do not introduce new OpenClaw compatibility features or make OpenClaw route/browser contracts the default architecture. New IM/gateway/routing work should use Hermes-aligned neutral names and profile/runtime boundaries; temporary OpenClaw wrappers must be thin, removable adapters.
...
- Hermes-aligned sidecar removal roadmap: [2026-05-11-hermes-aligned-sidecar-removal-roadmap.md](/e:/code/yzpd/workclaw/docs/plans/2026-05-11-hermes-aligned-sidecar-removal-roadmap.md)
...
- The primary local Hermes Agent reference checkout for WorkClaw architecture work is `references/hermes-agent` under this repo. Use that checkout first when comparing runtime, gateway, tool registry, toolset, MCP, browser, memory, skill, curator, or scheduler designs.

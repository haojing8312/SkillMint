...
- 如果要宣告“完全对齐”，仍需要把当前 plugin-host 侧的窄 completion-order 回归，与 `im_host` / 多渠道侧的执行级证据进一步收束成同一份阶段验收结论
...
- plugin-host runtime fixture 现在也已补上更窄的 completion-order 回归：
  - 同一轮最终答复会先出现 `final_chunk_queued`
  - 然后依次进入 `wait_for_idle -> idle_reached -> fully_complete -> dispatch_idle`
  - `dispatch_idle` 只能出现一次，且不会早于最终发送完成
...
- plugin-host 侧关于 `dispatch_idle` 的最终完成边界已经拿到更窄的持续回归证据

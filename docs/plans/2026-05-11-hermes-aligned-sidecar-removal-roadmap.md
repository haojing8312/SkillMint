...
**T2A partial result (2026-05-11):** WeCom public start/status/stop command paths now validate existing credentials and record/read the runtime-owned `wecom` status in `ImChannelHostRuntimeState` without defaulting to `/api/channels/start`, `/api/channels/health`, or `/api/channels/stop`. Legacy WeCom sidecar channel start/health/stop calls remain only in explicitly named `*_via_sidecar_*` compatibility helpers. This does not complete Feishu migration, channel diagnostics/catalog removal, browser provider work, or full sidecar removal.
